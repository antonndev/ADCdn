...
SSL_ENABLED="${ADPANEL_NODE_SSL_ENABLED:-0}"
SSL_DOMAIN="${ADPANEL_NODE_SSL_DOMAIN:-}"
SSL_CERT="${ADPANEL_NODE_SSL_CERT:-}"
SSL_KEY="${ADPANEL_NODE_SSL_KEY:-}"
...
provision_tls_certificate() {
  [ "${SSL_ENABLED:-0}" = "1" ] || return 0
  if [ -z "${SSL_DOMAIN:-}" ]; then
    warn "SSL enabled but no domain provided; skipping certificate provisioning."
    return 0
  fi

  # Already have a usable certificate? Nothing to do.
  if [ -n "${SSL_CERT:-}" ] && [ -n "${SSL_KEY:-}" ] && [ -s "$SSL_CERT" ] && [ -s "$SSL_KEY" ]; then
    log "TLS certificate already present for $SSL_DOMAIN."
    return 0
  fi

  log "Provisioning Let's Encrypt TLS certificate for $SSL_DOMAIN..."

  # certbot is the most portable ACME client across distros.
  if ! have certbot; then
    log "Installing certbot..."
    case "$PM" in
      apt) pkg_install certbot || warn "Could not install certbot via apt." ;;
      dnf|yum)
        pkg_install epel-release >/dev/null 2>&1 || true
        pkg_install certbot || warn "Could not install certbot via $PM." ;;
      apk) pkg_install certbot || warn "Could not install certbot via apk." ;;
      pacman) pkg_install certbot || warn "Could not install certbot via pacman." ;;
      zypper) pkg_install certbot || warn "Could not install certbot via zypper." ;;
    esac
  fi

  if ! have certbot; then
    warn "certbot is not available; the daemon will fall back to HTTP until a certificate exists at $SSL_CERT."
    return 0
  fi

  # HTTP-01 challenge needs port 80 reachable from the internet.
  open_firewall_port 80

  # Use the adaemon account email if the panel did not provide one.
  local acme_email="${ADPANEL_NODE_SSL_EMAIL:-admin@${SSL_DOMAIN}}"

  if certbot certonly --standalone --non-interactive --agree-tos \
      --preferred-challenges http \
      --register-unsafely-without-email \
      -d "$SSL_DOMAIN" >/dev/null 2>&1 \
    || certbot certonly --standalone --non-interactive --agree-tos \
      --preferred-challenges http \
      -m "$acme_email" \
      -d "$SSL_DOMAIN" >/dev/null 2>&1; then
    log "Certificate obtained for $SSL_DOMAIN."
  else
    warn "Failed to obtain a Let's Encrypt certificate for $SSL_DOMAIN."
    warn "Ensure $SSL_DOMAIN resolves to this server and port 80 is reachable, then re-run the installer."
    warn "The daemon will serve HTTP until the certificate at $SSL_CERT exists."
    return 0
  fi

  if [ ! -s "$SSL_CERT" ] || [ ! -s "$SSL_KEY" ]; then
    warn "certbot reported success but $SSL_CERT / $SSL_KEY are missing; the daemon will fall back to HTTP."
    return 0
  fi
...
install_cert_renew_hook() {
  # Make the daemon pick up renewed certificates automatically.
  local hook_dir="/etc/letsencrypt/renewal-hooks/deploy"
  mkdir -p "$hook_dir" 2>/dev/null || return 0
  cat > "$hook_dir/adaemon-reload.sh" <<'HOOK'
#!/bin/sh
# Restart ADaemon so it reloads the renewed TLS certificate.
if command -v systemctl >/dev/null 2>&1 && [ -d /run/systemd/system ]; then
  systemctl restart ADaemon.service 2>/dev/null || systemctl restart adaemon.service 2>/dev/null || true
elif command -v rc-service >/dev/null 2>&1; then
  rc-service adaemon restart 2>/dev/null || true
elif command -v service >/dev/null 2>&1; then
  service adaemon restart 2>/dev/null || true
fi
HOOK
  chmod +x "$hook_dir/adaemon-reload.sh" 2>/dev/null || true
}
...
stop_disable_remove_service() {
  # Stop, disable and delete the ADaemon service across init systems.
  if systemd_running; then
    systemctl disable --now "$SERVICE_NAME.service" >/dev/null 2>&1 || true
    systemctl disable --now "$ALIAS_SERVICE_NAME.service" >/dev/null 2>&1 || true
    rm -f \
      "/etc/systemd/system/$SERVICE_NAME.service" \
      "/etc/systemd/system/$ALIAS_SERVICE_NAME.service" \
      "/etc/systemd/system/adpanel-daemon.service" 2>/dev/null || true
    systemctl daemon-reload >/dev/null 2>&1 || true
    systemctl reset-failed "$SERVICE_NAME.service" "$ALIAS_SERVICE_NAME.service" >/dev/null 2>&1 || true
  elif have rc-service; then
    rc-service "$ALIAS_SERVICE_NAME" stop >/dev/null 2>&1 || true
    rc-update del "$ALIAS_SERVICE_NAME" default >/dev/null 2>&1 || true
    rm -f "/etc/init.d/$ALIAS_SERVICE_NAME" 2>/dev/null || true
  elif have service || [ -f "/etc/init.d/$ALIAS_SERVICE_NAME" ]; then
    service "$ALIAS_SERVICE_NAME" stop >/dev/null 2>&1 || true
    if have update-rc.d; then update-rc.d -f "$ALIAS_SERVICE_NAME" remove >/dev/null 2>&1 || true; fi
    if have chkconfig; then chkconfig --del "$ALIAS_SERVICE_NAME" >/dev/null 2>&1 || true; fi
    rm -f "/etc/init.d/$ALIAS_SERVICE_NAME" 2>/dev/null || true
  fi
}

remove_adaemon_user() {
  id adaemon >/dev/null 2>&1 || return 0
  log "Removing adaemon system user..."
  if have userdel; then
    userdel adaemon >/dev/null 2>&1 || true
  elif have deluser; then
    deluser adaemon >/dev/null 2>&1 || true
  fi
  # Remove a leftover dedicated group if it is now empty.
  if getent group adaemon >/dev/null 2>&1; then
    if have groupdel; then groupdel adaemon >/dev/null 2>&1 || true
    elif have delgroup; then delgroup adaemon >/dev/null 2>&1 || true
    fi
  fi
}
...
uninstall_adaemon() {
  ...
  log "ADaemon uninstall requested."

  # --- Confirmation -------------------------------------------------------
  # Interactive: ask. Non-interactive: require ADPANEL_DAEMON_ASSUME_YES=1 so a
  # piped invocation can never wipe a node without an explicit opt-in.
  if [ -t 0 ]; then
    local reply=""
    printf "This will PERMANENTLY remove ADaemon (/var/lib/node, services, certs).\n"
    printf "Type YES or 'adaemon' to confirm: "
    read -r reply
    case "$reply" in
      YES|adaemon) : ;;
      *) die "Confirmation did not match. Aborting; nothing was changed." ;;
    esac
  elif [ "${ADPANEL_DAEMON_ASSUME_YES:-0}" != "1" ]; then
    die "Refusing to uninstall non-interactively. Re-run with ADPANEL_DAEMON_ASSUME_YES=1 to confirm."
  fi

  # --- Detect the TLS cert name before deleting the config ----------------
  local cert_name="${ADPANEL_NODE_SSL_DOMAIN:-}"
  if [ -z "$cert_name" ] && [ -f "$NODE_DIR/config.yml" ]; then
    local cert_line
    cert_line="$(grep -oE '/etc/letsencrypt/live/[^/"'"'"' ]+' "$NODE_DIR/config.yml" 2>/dev/null | head -n 1 || true)"
    if [ -n "$cert_line" ]; then
      cert_name="${cert_line#/etc/letsencrypt/live/}"
    fi
  fi

  # --- Stop and remove services ------------------------------------------
  log "Stopping and removing ADaemon service..."
  stop_disable_remove_service

  # --- Remove the built binary -------------------------------------------
  rm -f "$BIN_DST" 2>/dev/null || true

  # --- Remove ADaemon directories (guarded) ------------------------------
  local dir
  for dir in "$NODE_DIR" "$GO_DIR"; do
    if [ -n "$dir" ] && [ "$dir" != "/" ] && [ -d "$dir" ]; then
      log "Removing $dir ..."
      rm -rf "$dir" 2>/dev/null || warn "Could not fully remove $dir; check permissions."
    fi
  done

  # --- Remove the certbot renewal hook and TLS certificate ---------------
  rm -f /etc/letsencrypt/renewal-hooks/deploy/adaemon-reload.sh 2>/dev/null || true
  if [ -n "$cert_name" ] && have certbot; then
    log "Deleting TLS certificate '$cert_name'..."
    certbot delete --cert-name "$cert_name" --non-interactive >/dev/null 2>&1 \
      || warn "Could not delete certbot certificate '$cert_name'; remove it manually if needed."
  fi
  ...
  # --- Remove log files ---------------------------------------------------
  rm -f /var/log/adaemon.log /var/log/adaemon.err "$LOG_FILE" 2>/dev/null || true

  log "ADaemon uninstalled."
  log "Left untouched on purpose: Docker, Go ($GO_INSTALL_ROOT), certbot, base packages, firewall rules."
}

choose_and_run() {
  local choice="${ADPANEL_DAEMON_CHOICE:-}"
  case "${1:-}" in
    --uninstall|--remove) choice="2" ;;
    --install)            choice="1" ;;
    --choice)             choice="${2:-}" ;;
  ...
  if [ -z "$choice" ]; then
    if [ -t 0 ] && [ -t 1 ]; then
      printf '\n=== ADPanel Daemon (ADaemon) ===\n'
      printf '  1) Install ADaemon\n'
      printf '  2) Uninstall ADaemon\n'
      printf 'Enter choice (1 or 2): '
      read -r choice
    else
      # Non-interactive (e.g. piped from the panel installer): keep installing.
      choice="1"
    fi
  fi
